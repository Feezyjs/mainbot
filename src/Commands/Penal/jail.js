...
exports.run = async (Moderation, message, args) => {
    ...
    message.channel.send(Embed.setColor("RANDOM").setDescription(`${Member} (\`${Member.id}\`) üyesi cezalıya atıldı. (\`#${NewPenal.Id}\`)`).setTimestamp()).then(x => x.delete({timeout: 10000}))
        message.react("✅")
