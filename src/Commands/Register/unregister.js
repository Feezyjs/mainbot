...
exports.run = async (Moderation, message, args) => {
    ...
    message.channel.send(new MessageEmbed().setColor("RANDOM").setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true, size: 2048 })).setDescription(`${Member} adlı üye kayıtsıza atıldı!`)).then(x => x.delete({timeout: 10000}));
