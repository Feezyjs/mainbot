...
exports.run = async(client, message, [ option ]) => {
    ...
    if (option === "stats") {
        StatsModel.find({}).exec((err, data) => {
            ...
            const GeneralVoice = data.sort((x, y) => y.TotalVoice - x.TotalVoice).slice(0, 10).map((_data, index) => `**\`${index + 1}.\`** <@${_data.Id}> \`${moment.duration(_data.TotalVoice).format("H [saat, ] m [dakika]")}\``).join("\n");
            const GeneralMessage = data.sort((x, y) => y.TotalMessage - x.TotalMessage).slice(0, 10).map((_data, index) => `**\`${index + 1}.\`** <@${_data.Id}> \`${_data.TotalMessage} mesaj\``).join("\n");
        ...
        const FindUser = Members.find((item) => item.Id == message.author.id);
